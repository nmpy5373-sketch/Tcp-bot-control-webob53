...
def get_player_info(uid):
    try:
        url = f"https://info-ob49.onrender.com/api/account/?uid={uid}&region=bd"
...
📱 Player will see:
"SAMI GAMER wants to join your team"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]SAMI GAMER [00FF00]___ SAMI GAMER____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @SAMI_GAMER\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
[C][B][00FFFF]━━━━━━━━━━━
[C][B][FFFFFF]Developer     : SAMI GAMER
"""
    # Send main info
...
def get_level_info(player_id):
    url = f"https://info-ob49.onrender.com/api/account/?uid={player_id}&region=bd"
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]SAMI GAMER [00FF00]!!
"""
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="SAMI", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="SAMI", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde SAMI CODEX ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="SAMI", title_id=title_id)
...
[4DA6FF]│show all cmd-/help
[FFFFFF]├ Owner : [FFFF00]﷽ㅤ—͟͞͞SAMI፝֟፝ㅤ!!
[FFFFFF]└ TG : [00FF00]@SAMI✨
...
[4DA6FF]│
[FFFFFF]├ Stop evo emote : [FF0000]@sevos
...
[FFFFFF]• 10"""
                                ...
                                # Real IDs
                                bundle_ids = {
                                    "1": "914000002", "2": "914000003",
                                    "3": "914038001", "4": "914039001",
                                    "5": "914042001", "6": "914044001",
                                    "7": "914047001", "8": "914047002",
                                    "9": "914048001", "10": "914050001",
                                    "11": "914051001",
                                    "12": "914053001"
                                }
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by SAMI GAMER
/bio Level 70 | Pro Player
/bio Add me: SAMI GAMER
...
[b][i][FFC0CB]youtube: SAMI CODEX[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @sami Codex[/b]

[b][c][FFA500]telegram contact: @Sami codex[A52A2A]
...
# GLOBAL BLOCKED NAMES
                        BLOCKED_NAMES = ["maruf", "SAMI", "mg-king", "mg_king", "mgking", "mg king"]  # Protected names
...
# In TcPChaT function, update /status command:
                        if inPuTMsG.strip().startswith('/status '):
                            ...
                            try:
                                import requests
                                info_url = f"https://info-ob49.onrender.com/api/account/?uid={target_uid}&region=bd"
                                    ...
                                    if cache_data:
                                        ...
                                        if "IN ROOM" in cache_data['status']:
                                            ...
                                            if 'leader_id' in cache_data:
                                                ...
                                                try:
                                                    leader_api = f"https://info-ob49.onrender.com/api/account/?uid={leader_uid}&region=bd"
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "hi"):
                            ...
                            header = f"[c][00FF00]Hey {player_name} [00FFFF]Welcome To SAMI GAMER's BOT"
...
[FFFF00]━━ EVO SYSTEM ━━
[FF00FF]│ [00FF00]/evo [uid] [FFFFFF][num 1-21]
[FF00FF]│ [00FF00]/e [uids] [FFFFFF][emtcd]
[FF00FF]│ [00FF00]@evos [uid][FFFFFF] [evo Cycle]
[FF00FF]└ @sevo[FFFFFF]s
[FFFFFF]Player Emote Without Bot
[FF00FF]/me [tc] [uid] [1-420]


[FFFF00]━━ ANIMATION & MAGIC ━━
[FF00FF]│ [00FF00]/ti[00FF00]tle
[FF00FF]│ [00FF00]/bun[00FF00]dle
[FF00FF]│ [00FF00]/bundle [FFFFFF][NUMBER]
...
[FF0000]└ [00FF00]/s1, s2, s3, s4, s5  [FFFFFF][UID]
...
[FFFF00]━━━━━━━━━━━━━━━━━"""
                            ...
                            # ───── Commands 51-62 ─────
                            help_6 = """
[C][B][32CD32]═⚡Aᴅᴍɪɴ Cᴏᴍᴍᴀɴᴅꜱ⚡═[7CFC00][B]
...
[FFFF00]━━━━━━━━━━━━"""
                            ...
                            # Footer
                            footer = """[C][B][FFFF00]═ Bᴀsɪᴄ ᴀɴᴅ Aɴʏ_Rᴏʟʟs ═[FFFFFF][B]
